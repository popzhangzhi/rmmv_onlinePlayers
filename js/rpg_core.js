...
/**
 * 检查url是否含有string，检查nw 参数中是否包含，返回bool
...
Graphics.initialize = function(width, height, type) {
    ...
    // 测试是否支持混合 饱和模式，并且复制到this
    this._testCanvasBlendModes();
    // 修改现有元素，把所有zIndex大于0的都变成0
    ...
    // 增加全局监听缩放事件，并防止video不自动播放
    this._setupEventHandlers();
    // 字体提前预载成功后，写入_fontLoaded
...
Graphics._createAllElements = function() {
    ...
    // 创建video
    this._createVideo();
    // 创建上层画布，类似遮罩，也做输入事件的捕获层
    this._createUpperCanvas();
    // 创建渲染，通过pixi，根据render来决定渲染方式
    this._createRenderer();
    // 通过 FPSMeter.js 来创建fpsmeter 默认关闭
    this._createFPSMeter();
    // 创建显示渲染模式层。webgl canvas
    this._createModeBox();
    // 提前预加载字体，创建1*1大小的.文字的透明div来加载
...
/**
 * 更新拉伸，根据_stretchEnabled，如果可以拉伸，找到长宽中最小的拉伸比进行拉伸
 * @static
...
Input.clear = function() {
    ...
    this._dir4 = 0;
    this._dir8 = 0;
    // 首选轴，当dir4时，同时按下上和左，需要一个首选轴来决定怎么走
    this._preferredAxis = '';
    this._date = 0;
};
